...
import 'package:manzel/my_properties/properties_bloc.dart';
...
class _MyPropertiesWidgetState extends State<MyPropertiesWidget> {
  ...
  Future<void> bookedPropertiesCall() async {
    ...
    MyPropertiesBloc myPropertiesBloc = MyPropertiesBloc();
  Future<void> getBookMarkedPropertiesCall() async {
    isInternetAvailable = await isInternetConnected();
    if ((isInternetAvailable ?? false) && loggedIn) {
      myPropertiesBloc.getProperties(currentUserUid);
    } else if ((!(isInternetAvailable ?? false)) && loggedIn) {
                ...
                child: DefaultTabController(
                  ...
                  child: Column(
                    children: [
                      ...
                      Expanded(
                        child: BlocBuilder<MyPropertiesBloc, PropertiesState>(
                          bloc: myPropertiesBloc,
  ...
  Widget buildItemCard  (PropertiesState state , ) {
    return ListView.builder(
      shrinkWrap: true,
      padding: EdgeInsets.zero,
      scrollDirection: Axis.vertical,
      itemCount: state.bookmarkedProperties?.length,
      itemBuilder:
                      ...
                      children: [
                        ...
                        InkWell(
                          onTap: () async {
                            ...
                            logFirebaseEvent(
                                'Icon_Backend-Call');
                            myPropertiesBloc.removeProperty(currentUserUid, bookmarkedPropertiesItem);
                            if ((state.result
                                ?.statusCode) ==
                                200) {
                              logFirebaseEvent(
  ...
  buildSecondTab(state) {
    if(state.state == ApiResponseState.loading){
      return  Center(
        child: SizedBox(
          width: 50,
          height: 50,
    ...
    }else if(state.state == ApiResponseState.loaded) {
      if (state.bookmarkedProperties?.isEmpty ??
          false) {
        return Center(
          child: Container(
            width: MediaQuery.of(context).size.width,
      ...
      } else if ((state.bookmarkedProperties != null) &&
          (state.bookmarkedProperties?.isNotEmpty ??
              false)) {
        return buildItemCard(state);
      } else if (state.result
          ?.statusCode ==
          403) {
        unAuthorizedUser(context, mounted);
